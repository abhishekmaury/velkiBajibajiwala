...
export class ClassicHeaderComponent implements OnInit {
  ...
  constructor(private authServe: AuthserviceService, private dataserve: DataHandlerService, private router: Router,  private fingerprintService: FingerprintService) { }
  ...
  ngOnInit(): void {
    ...
    this.authServe.sendLoggedData.subscribe((re: any) => {
      if (re.data.login == true) {
        ...
        this.afterLogin = true;
        this.beforeLogin = false;
        this.agentType = re.data.user.agentType;
        this.country = re.data.user.country;
    ...
    if (whatdt) {
      this.footerLinkss = JSON.parse(whatdt);
    } else {
      this.gefooterLinks();
    }
 ...
 //              this.userBalance = gms.data.balance;
